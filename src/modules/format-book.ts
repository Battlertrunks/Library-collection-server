...
export function formatBook(book: BookListing, result: any): BookListing {
  ...
  book.authors          = result?.items[0]?.volumeInfo?.authors?.join(",") || "COULD NOT FIND";
  book.thumbnail_url    = result?.items[0]?.volumeInfo?.imageLinks?.thumbnail || null; // use a placeholder image?
